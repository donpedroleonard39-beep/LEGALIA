...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, pendingInvite, initialMode = 'signup' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // - silently pretending sign-in/sign-up/reset succeeded would be misleading.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm" style={{ background: 'rgba(10,17,29,.78)' }}>
      ...
      >
        ...
        {mode === 'reset' && (
          <p className="mt-4 mb-1 text-[12px] leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            Enter the email address on your account. We will send a link to reset your password.
          </p>
        )}

        {errorMessage && (
          <div className="mb-3 mt-3 p-2.5 rounded-lg text-[12px]" style={{ background: 'rgba(189,81,75,.1)', border: '1px solid rgba(189,81,75,.3)', color: 'var(--alert-red)' }}>
            {errorMessage}
          </div>
        )}

        {mode === 'reset' && resetSent ? (
          <div className="mt-3 p-3 rounded-lg text-[12px] leading-5" style={{ background: 'var(--gold-soft)', border: '1px solid rgba(208,173,114,.35)', color: 'var(--text-main)' }}>
            If an account exists for <strong>{email}</strong>, a password reset link is on its way. Check your inbox, and your spam folder if it does not show up shortly.
          </div>
        ) : (
          /* Email Form */
