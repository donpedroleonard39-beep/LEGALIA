...
export async function saveMatter(
  ...
  await addTimelineEvent(newId, {
    ...
    type: 'filing',
    summary: `Matter opened with Suit No. ${matterData.suitNumber}.`,
...
/**
 * Renamed from deleteMatterById to deleteMatter to match the UI imports.
 */
...
export async function generateInviteLink(
  ...
  const invite: MatterInvite = {
    ...
    email: '', // Not tied to a specific email until they accept
    invitedBy: '', // Simplified for this view
    invitedByName: '',
    status: 'pending',
    permission,
    token: genToken(),
  ...
  // Construct the absolute URL based on the current domain
  const baseUrl = window.location.origin;
  return `${baseUrl}/invite/${matterId}/${invite.id}?token=${invite.token}`;
}
...
export async function fetchTimelineEvents(matterId: string): Promise<TimelineEvent[]> {
  ...
  const events: TimelineEvent[] = snap.docs.map(d => ({ id: d.id, ...d.data() } as TimelineEvent));
...
export async function fetchMatterDocuments(matterId: string): Promise<MatterDocument[]> {
  ...
  const snap = await getDocs(subCol);
  const docs = snap.docs.map(d => ({ id: d.id, ...d.data() } as MatterDocument));
...
export async function notifyMatterTeam(
  ...
  await Promise.all(
    recipients.map((uid) => {
      ...
      return addDoc(collection(db, NOTIFICATIONS_COLLECTION), notif);
    })
  );
}
